...
public class AdminDao implements InterfaceDao<Admin>{
	...
	public void changeState(int idAdmin, char state){
	...
	public Admin findByEmail(String val) {
		...
		List<Admin> listContact = jdbcTemplate.query(sql, new RowMapper<Admin>() {
			...
			public Admin mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				Admin aContact = new Admin.BuildAdmin(rs.getString("nombre"))
						.setLastname(rs.getString("apellidoAdmin"))
						.setEmail(rs.getString("correoAdmin"))
						.setPassword(rs.getString("contrasenaAdmin"))
						.build();		
				aContact.setIdAdmin(rs.getInt("idAdmin"));
				return aContact;
			}
